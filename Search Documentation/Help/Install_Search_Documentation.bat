...
HelpLibraryManagerLauncher.exe /product "VS" /version "100" /locale zh-cn /uninstall /silent /vendor "Vendor Name" /productName "A Search Documented Class Library" /mediaBookList "A Search Documented Class Library" > NUL
...
REM build time.
IF EXIST "Search_Documentation.msha" COPY /Y "Search_Documentation.msha" HelpContentSetup.msha
...
REM Uninstall first in case it is already there.  If not, it won't install below.  We'll ignore any error output
REM by redirecting it to NUL.
HelpLibraryManagerLauncher.exe /viewerVersion %1  /locale zh-cn /wait 0 /operation uninstall /vendor "Vendor Name" /productName "A Search Documented Class Library" /bookList "A Search Documented Class Library" > NUL

REM Install the new content.
HelpLibraryManagerLauncher.exe /viewerVersion %1  /locale zh-cn /wait 0 /operation install /sourceUri "%CD%\Search_Documentation.msha"
